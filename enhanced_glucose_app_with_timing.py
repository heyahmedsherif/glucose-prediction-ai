...
class UltimateGlucosePrediction:
    """Ultimate glucose prediction with advanced fiber-timing integration."""
    ...
    def __init__(self):
        ...
        # Advanced fiber response profiles
        ...
        # Fiber effectiveness by timing (based on analysis)
        self.fiber_timing_effectiveness = {
            'breakfast_early': 1.5,  # 6-9 AM breakfast
            'breakfast_late': 1.0,
            'lunch_optimal': 1.2,    # 12-2 PM lunch
            'lunch_other': 1.0,
            'dinner': 0.8,
            'first_meal_bonus': 1.3  # Extra effectiveness for first meal
        }
    ...
    def get_dynamic_fiber_recommendation(self, meal_type: str, hour: int, 
        ...
        """Calculate dynamic fiber recommendation based on context."""
        
        # Base recommendation: 15-20% of carbs as fiber
        base_fiber = carbs * 0.18
        
        # Timing adjustments
        if meal_type == 'breakfast' and 6 <= hour <= 9:
            # Dawn phenomenon boost
            base_fiber *= 1.5
        elif meal_type == 'lunch' and 12 <= hour <= 14:
            # Optimal fiber window - less needed
            base_fiber *= 0.9
        elif hour >= 20:
            # Late evening - reduce fiber for digestion
            base_fiber *= 0.8
        
        # First meal adjustment
        if is_first_meal:
            base_fiber *= 1.3  # Boost for first meal benefits
        
        # Diabetic status adjustments
        profile = self.fiber_response_profiles[diabetic_status]
        base_fiber *= profile['base_sensitivity'] / 0.6  # Normalize to Normal baseline
        
        # Reasonable bounds
        min_fiber = max(2, carbs * 0.05)  # At least 5% of carbs
        max_fiber = min(25, carbs * 0.35)  # Max 35% of carbs
...
def load_predictor():
    ...
    return UltimateGlucosePrediction()
...
def create_fiber_effectiveness_chart(fiber_range: range, meal_inputs: Dict[str, float], 
                                   ...
                                   predictor: UltimateGlucosePrediction):
